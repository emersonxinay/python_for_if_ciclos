...
def read_data(filepath="./palabras/palabras.txt"):
    words = []
...
def run():
    data = read_data(filepath="./palabras/palabras.txt")
...
""")
        ...
        try:
            letter = input(
                "Ingresa una letra y presiona Enter: ").strip().upper()
            assert letter.isalpha(), input(
                "¡SOLO PUEDES INGRESAR LETRAS!, Presiona la tecla Enter para continuar.")
            assert len(letter) == 1, input(
...
while True:
    try:
        ...
        if int(opcion) == 1:
            ...
            input(
                "Debes ingresar el número 1 para jugar. Presiona la tecla Enter para reintentar.")
        assert len(str(opcion)) == 1, input(
            "¡No puedes ingresar varias letras o números!, Presiona la tecla Enter para reintentar.")
